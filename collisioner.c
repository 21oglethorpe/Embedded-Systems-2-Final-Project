   ...
   while (!(playerc >=0 && playerc <16 && playerr>49 && playerr <64)) {
      ...
      if (GYRO_Int2Status(&myDevice) != 0) {
         ...
         if(within_neg(yAxis)) //north
             ...
             if(n==1 && !collision(playerc, playerr-1) ) {move(1, &playerc, &playerr);}
             else if(s==1&& !collision(playerc, playerr+1) ){ move(3, &playerc, &playerr);}
              if(w==1 && !collision(playerc-1, playerr)) { move(4, &playerc, &playerr);}
              else if(e==1 && !collision(playerc+1, playerr)){ move(2, &playerc, &playerr);}
              xil_printf("%d\n", n);
              xil_printf("%d\n", e);
              xil_printf("%d\n", s);
              xil_printf("%d\n", w);
